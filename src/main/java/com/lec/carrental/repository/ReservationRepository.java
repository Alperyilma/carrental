...
//    @Query("SELECT new com.lec.carrental.dto.ReservationDTO(r) FROM Reservation r")
    List<ReservationDTO> findAllBy();

    Optional<ReservationDTO> findByIdOrderById(Long id) throws ResourceNotFoundException;

//    @Query("SELECT new com.lec.carrental.dto.ReservationDTO(r) FROM Reservation r WHERE r.id = ?1 and r.userId = ?2")
    Optional<ReservationDTO> findByIdAndUserId(Long id, User user) throws ResourceNotFoundException;

//    @Query("SELECT new com.lec.carrental.dto.ReservationDTO(r) FROM Reservation r WHERE r.userId.id = ?1")
